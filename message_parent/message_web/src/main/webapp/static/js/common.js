...
var E3 = {
    ...
    initItemCat : function(data){
    	$(".selectItemCat").each(function(i,e){
    		var _ele = $(e);
    		if(data && data.cid){
    			_ele.after("<span id='spanColumn' style='margin-left:10px;'>"+data.cid+"</span>");
    		}else{
    			_ele.after("<span id='spanColumn' style='margin-left:10px;'></span>");
    		}
    		_ele.unbind('click').click(function(){
    			...
    			.window({
    			    ...
    			    onOpen : function(){
    			    	var _win = this;
    			    	$("ul",_win).tree({
    			    		url:'/item/cat/list',//发送请求
    			    		animate:true,
    			    		onClick : function(node){
    			    			if($(this).tree("isLeaf",node.target)){
    			    				// 填写到cid中
    			    				_ele.parent().find("[name=cid]").val(node.id);
    			    				_ele.parent().find("[name=myColumn]").remove();
